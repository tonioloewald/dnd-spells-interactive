...
const queueRender = (elt) => {
  if (! awaitingUpdate.length) {
    requestAnimationFrame(() => {
      console.time('render elements');
      for(let i = 0; i < awaitingUpdate.length; i++) {
        awaitingUpdate[i].render();
      }
      awaitingUpdate.splice(0);
      console.timeEnd('render elements');
    })
  }
  if (! awaitingUpdate.includes(elt)) {
    awaitingUpdate.push(elt);
  }
};

const triggerChange = (elt) => elt.dispatchEvent(new Event('change'));
      ...
      if (attributeNames.length) {
        ...
        const observer = new MutationObserver((mutationsList) => {
          ...
          mutationsList.forEach((mutation) => {
            if(mutation.attributeChange === 'value') triggerChange(this);
            if(attributeNames.includes(mutation.attributeName)) triggerRender = true;
          });
          if (triggerRender) queueRender(triggerChange);
        });
            ...
            set(value){
              if(typeof attributes[attributeName] === 'boolean') {
                if (value !== this[attributeName]) {
                  ...
                  if (attributeName === 'value') triggerChange(this);
                  queueRender(this);
                }
              } else {
